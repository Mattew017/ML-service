...
class MetricTable(Base):
    ...
    train_metrics: Mapped[list['TrainMetricsTable']] = relationship(
        'TrainMetricsTable', back_populates='metric'
    )
    evaluation_metrics: Mapped[list['EvaluationMetricsTable']] = relationship(
        'EvaluationMetricsTable', back_populates='metric'
    )
...
class TrainMetricsTable(Base):
    ...
    metric: Mapped[MetricTable] = relationship(
        'MetricTable', back_populates='train_metrics',
        lazy='joined'
    )
...
class EvaluationMetricsTable(Base):
    ...
    metric_id: Mapped[int] = mapped_column(ForeignKey('metrics.id'))
    model_id: Mapped[int] = mapped_column(ForeignKey('model_evaluations.id'))
    value: Mapped[float]

    metric: Mapped[MetricTable] = relationship(
        'MetricTable', back_populates='evaluation_metrics',
        lazy='joined'
    )
